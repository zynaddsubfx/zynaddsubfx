...
void Chorus::out(REALTYPE *smpsl,REALTYPE *smpsr){
	...
	for (i=0;i<SOUND_BUFFER_SIZE;i++){
	    smpsl[i]=smpsl[i]*v1+efxoutl[i]*panning*v2*2.0;
	    smpsr[i]=smpsr[i]*v1+efxoutr[i]*(1.0-panning)*v2*2.0;
	};
    } else {//System effect
	for (i=0;i<SOUND_BUFFER_SIZE;i++){
	    efxoutl[i]*=2.0*volume*panning;
	    efxoutr[i]*=2.0*volume*(1.0-panning);
	    smpsl[i]=efxoutl[i];
	    smpsr[i]=efxoutr[i];
	};
    };
