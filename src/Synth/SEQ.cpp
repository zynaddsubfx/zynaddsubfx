    ...
    watchOut(m, watch_prefix, "out")
{
    ...
    if(!seqpars.speedratio) {
        ...
        duration = 60.0f / limit(((float(time.bpm)) * seqpars.speedratio), 0.001f, 1000.0f);
        tRef = time.tRef;
    }
    ...
    if ( ! seqpars_.time || seqpars_.last_update_timestamp == seqpars_.time->time())
    {
        if((seqpars_.speedratio)<0.01) { 
            if (seqpars_.freq < 0.001) {
                //~ printf("z109 seqpars_.freq = %f\n", seqpars_.freq);
                duration = 1000.0f;
            }  
            duration = 1.0/seqpars_.freq;  
            //~ printf("z113 seqpars_.freq = %f\n", seqpars_.freq);
        } else {
            duration = 60.0f / limit(((float(time.bpm)) * seqpars_.speedratio), 0.001f, 10000.0f);
        }
    }
