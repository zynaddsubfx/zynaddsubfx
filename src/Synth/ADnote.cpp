...
void ADnote::setupVoice(int nvoice)
{
    ...
    NoteVoicePar[nvoice].basefunc =
        getBaseFunction(pars.VoicePar[vc].OscilGn->Pcurrentbasefunc);
    {
        char bfp = pars.VoicePar[nvoice].OscilGn->Pbasefuncpar;
...
void ADnote::legatonote(const LegatoParams &lpars)
{
    ...
    for(int nvoice = 0; nvoice < NUM_VOICES; ++nvoice) {
        ...
        //WaveTable does not need the precomputed samples, but the base func
        NoteVoicePar[nvoice].basefunc =
           getBaseFunction(pars.VoicePar[nvoice].OscilGn->Pcurrentbasefunc);
        if(voiceFilter) {
...
{
    // TODO: enable sync modulation: 348e8027685a7d9d7949e4096570a5e8076bd857
    Voice& vce = NoteVoicePar[nvoice];
    ...
    if(vce.FMVoice >= 0)
        ...
        for(int k = 0; k < vce.unison_size; ++k) {
            float *tw = tmpwave_unison[k];
            memcpy(tw, NoteVoicePar[vce.FMVoice].VoiceOut,
        	   synth.bufferbytes);
        }
    else
        //Compute the modulator and store it in tmpwave_unison[][]
        for(int k = 0; k < vce.unison_size; ++k) {
            int    poshiFM  = vce.oscposhiFM[k];
            float  posloFM  = vce.oscposloFM[k];  // TODO: Fix cast if you use int here: e2a29b2d60ac4ecb175daf70f949c4810f63eca7
            int    freqhiFM = vce.oscfreqhiFM[k];
            float  freqloFM = vce.oscfreqloFM[k];
            float *tw = tmpwave_unison[k];
            ...
            for(i = 0; i < synth.buffersize; ++i) {
                tw[i] =
                    (vce.FMSmp[poshiFM] * (1.0f - posloFM)
                     + vce.FMSmp[poshiFM + 1] * posloFM);
                posloFM += freqloFM;
                if(posloFM >= 1.0f) {
    ...
    unsigned char Pcurbasefunc = pars.VoicePar[nvoice].OscilGn->Pcurrentbasefunc;
    ...
    for(int k = 0; k < vce.unison_size; ++k) {
        ...
        for(int i = 0; i < synth.buffersize; ++i) {
            ...
            float par = std::max(0.0f, std::min(1.0f, (tw[i]/vce.FMSmpMax) + vce.basefuncpar));
//          if(par != (tw[i]/vce.FMSmpMax) + vce.basefuncpar)
//              printf("Error: %.2f != %.2f\n", par, (tw[i]/vce.FMSmpMax) + vce.basefuncpar);
