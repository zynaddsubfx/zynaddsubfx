...
inline void ADnote::ComputeVoiceOscillatorWaveTableModulation(int nvoice)
{
    ...
    for(int k = 0; k < vce.unison_size; ++k) {
        if(!vce.OscilSmp.isWaveTable) {
           printf("\n\n assertion isWaveTable would have failed \n");
           continue; 
        }
        ...
        for(int i = 0; i < synth.buffersize; ++i) {
            ...
            if(semantic >= wt->size_semantics() - 1)
                semantic = wt->size_semantics() - 2;
            ...
            const Tensor1<WaveTable::float32>& waveB = wt->getWaveAt(freq, semantic+1);
